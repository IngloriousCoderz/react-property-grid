...
const ListItem = ({schema, data, title, path, canRemove}) => (
  <PropertyEditor
    schema={schema.items}
    data={data}
    title={`${schema.items.title || schema.title || title}[${last(path)}]`}
    description={schema.items.description || schema.description}
    path={path}
    canRemove={canRemove} />
)
...
const ArrayEditor = ({schema, data, title, description, path, required, expanded, toggleExpanded, canEditKey, setValue, canRemove}) => {
  ...
  return (
    <div>
      ...
      {expanded
        ? <SortableList
            helperClass='react-property-grid-dragging-item'
            lockAxis={'y'}
            // pressDelay={200}
            useDragHandle={true}
